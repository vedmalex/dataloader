 ...
 */
import 'jest'
import DataLoader, { Options } from '..'

function idLoader<K, C = K>(
  options?: Options<K, K, C>,
): [DataLoader<K, K, C>, Array<Array<K>>] {
  const loadCalls = []
...
describe('Represents Errors', () => {
  it('Resolves to error to indicate failure', async () => {
    const loadCalls = []
    const evenLoader = new DataLoader((keys: Array<number>) => {
      loadCalls.push(keys)
...
describe('It is resilient to job queue ordering', () => {
  ...
  it('can call a loader from a loader', async () => {
    const deepLoadCalls = []
    const deepLoader = new DataLoader<Array<string>, Array<string>>(keys => {
