...
export default class DataLoader<K, V, C = K> {
  ...
  _batchScheduleFn: (scheduler: () => void) => void
  _cacheKeyFn: (key: K) => C
  ...
  load(key: K): Promise<V> {
    ...
    batch.keys.push(key)
    var promise = new Promise<V>((resolve, reject) => {
   ...
   *
   */
  loadMany(keys: Array<K>): Promise<Array<V | Error>> {
    if (!isArrayLike(keys)) {
      throw new TypeError(
...
var resolvedPromise: Promise<unknown>
...
function getValidMaxBatchSize(
  options?: Partial<Options<any, any, any>>,
): number {
...
function getValidCacheKeyFn<K, C>(
  options?: Partial<Options<K, any, C>>,
): (key: K) => C {
...
function getValidCacheMap<K, V, C>(
  ...
  if (cacheMap !== null) {
    var cacheFunctions: Array<keyof CacheMap<C, Promise<V>>> = [
      'get',
      'set',
      'delete',
      'clear',
    ] as unknown as Array<keyof CacheMap<C, Promise<V>>>
  ...
  return cacheMap as CacheMap<C, Promise<V>>
}

// Private
function isArrayLike(x: { length: number }) {
  return (
    typeof x === 'object' &&
    x !== null &&
